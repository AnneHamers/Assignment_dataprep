...
Original file is located at
    https://colab.research.google.com/drive/1I7pos14e0Q2O_29OwUBIoa1XmNHf-aB1
...
Parsing is translating the rows of data in understandable lines of information. Through understanding how datapoints are separated from each other and how the columns within those datapoints are separated, you can distangle the lines of data into readable information, in separate rows and columns.
...
- Which municipality_name has the highest amount of total_reported in the dataset?
- Which security_region_name has the highest amount of deceased in the dataset?
- Of total_reported, how many resulted in a  hospital_admission?


**7. Note another metadata property that you would like to know about that's not listed here.**

Duplicate values & total datapoints per municipality
...
String is value that contains text. You cannot calculate with a string.

**11. Explain, in your own words, what a date string is.**

A date string is the text value for a specific date.

**12. Explain, in your own words, why we converted date strings to a `datetime.date` type.**

When the date string was still a string, we could not calculate with it, it was just a text box. Once converted to the datetime.date type, the computer understands the values of the specified year, month and day and is able to calculate with it. Therefore it was able to retrieve the first and last date of publication and also to calculate how many days are between the first and last day.
...
It is not possible to have a negative number for reported new cases. You cannot substract a newly reported case, only add.
"""
...
We could instead of -1 add 0 to fill in the missing values. 
...
This method is more accurate than the previous method.
...
It will not work well when the surrounding dates are too far apart/when there are too many missing values or when the amount of newly reported cases is simply too unpredictable and all over the place. Using this method will then be highly unreliable. 

Example: day 1 reported 200 new cases, day 2 - ? cases, day 3 - 10 new cases, day 4 - ?, day 5 - ? and day 6 - 75 cases. Because half the dates have a missing amount of newly reported cases, it becomes harder to predict the values. Furthermore, the dates with an amount of newly reported cases also show no clear trend.



It will work well when a limited amount of values are missing or when the amount of newly reported cases show no big differences on a day-to-day basis (when the graph would be more "smooth").

Example: day 10 reported 50 new cases, day 11 - 60 cases, day 12 - 70 cases, day 13 - ? cases, day 14 - 90 cases, day 15 - 100 cases. It is then very likely that the missing value on day 13 is equal or close to 80, which will be in line with the increase in amount of newly reported cases every day.
